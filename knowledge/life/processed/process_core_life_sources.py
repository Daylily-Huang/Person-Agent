...
LOG = OUT / "life_processing.log"

OCR_DOCS = [
    {
        "id": "01_ziping_zhenquan",
        "pattern": "子平真诠*.pdf",
        "role": "八字主框架",
    },
    {
        "id": "02_zhouyi_yizhu",
        "pattern": "国学经典译注丛书 周易译注*.pdf",
        "role": "周易卦义与问题推演",
    },
]
...
def extract_text_doc(doc: dict) -> None:
    ...
    notes = [
        f"# Quality Notes - {source.name}",
        "",
        f"- Method: pypdf text extraction",
        f"- Total pages: {pages}",
...
def write_source_inventory() -> None:
    ...
    (OUT / "core_source_inventory.json").write_text(json.dumps(rows, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = ["# Life Core Source Inventory", "", "| ID | Source | Role | Pages | Size MB |", "|---|---|---|---:|---:|"]
    for row in rows:
        lines.append(f"| `{row['id']}` | `{row['source']}` | {row['role']} | {row['pages']} | {row['size_mb']} |")
    (OUT / "core_source_inventory.md").write_text("\n".join(lines) + "\n", encoding="utf-8")


def main() -> int:
    OUT.mkdir(parents=True, exist_ok=True)
    LOG.write_text(f"[{now()}] life core processing started\n", encoding="utf-8")
    write_source_inventory()
    for doc in OCR_DOCS:
        process_ocr_doc(doc)
    for doc in TEXT_DOCS:
        extract_text_doc(doc)
    log("life core processing complete")
    return 0
